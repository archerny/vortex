...
[#](./ftapi_protocol.md#2042)
 协议设计
---------------------------------------------------------------------------

协议数据包括协议头以及协议体，协议头固定字段，协议体根据具体协议决定。

### [#](./ftapi_protocol.md#8205)
 协议头
...
### [#](./ftapi_protocol.md#9850)
 协议体

#### [#](./ftapi_protocol.md#8785)
 Protobuf 协议请求包体结构
...
*   包体格式类型请求包由协议头 nProtoFmtType 指定，OpenD 主动推送格式在 [InitConnect](./ftapi_init.md#1515)
     设置。
...
*   **使用**
    ...
    根据[初始化链接](./ftapi_init.md#1990)
    返回的心跳保活间隔时间，向 OpenD 发送保活协议
    

[#](./ftapi_protocol.md#2846)
 加密通信流程
-----------------------------------------------------------------------------

*   若 OpenD 配置了加密，[InitConnect](./ftapi_init.md#1515)
     初始化连接协议必须使用 [RSA](./qa_other.md#4601)
     公钥加密，后续其他协议使用 InitConnect 返回的随机密钥进行 AES 加密通信。
*   OpenD 的加密流程借鉴了 SSL 协议，但考虑到一般是本地部署服务和应用，简化了相关流程，OpenD 与接入 Client 共用了同一个 [RSA](./qa_other.md#4601)
     私钥文件，请妥善保存和分发私钥文件。
*   可到这个 [网址](http://web.chacuo.net/netrsakeypair)
     在线生成随机 [RSA](./qa_other.md#4601)
     密钥对，密钥格式必须为 PCKS#1，密钥长度 512，1024 都可以，不要设置密码，将生成的私钥复制保存到文件中，然后将私钥文件路径配置到 [OpenD 配置](./opend_opend-cmd.md#8799)
...
← [通用定义](./ftapi_common.md) [OpenD 相关](./qa_opend.md)
 →

[底层协议介绍](./ftapi_protocol.md)

*   [协议请求流程](./ftapi_protocol.md#2892)
    
*   [协议设计](./ftapi_protocol.md#2042)
    
*   [心跳保活](./ftapi_protocol.md#2603)
    
*   [加密通信流程](./ftapi_protocol.md#2846)
    
*   [RSA 加解密](./ftapi_protocol.md#5219)
    
*   [AES 加解密](./ftapi_protocol.md#9339)