...
> **创建日期**：2026-04-13
> **最后更新**：2026-04-23（随 import-consistency.md v2.2 更新：Phase 3 Commit A 已上线 — fail-fast cleanup 接入 executor + recovery runner；409 SyncConflictException 已在 `BrokerSyncController` 中映射）
> **状态**：✅ 表结构与 Entity 已实现（DB 迁移 V19 + V22-V24 + V28）；🔧 应用层行为已接入 fail-fast cleanup + 409；仍待 Commit B 删除 v1 桥接方法与 `resumeSync` 端点，详见 [import-consistency.md](./import-consistency.md)
